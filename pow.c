...
unsigned char *serialize_block(const TransactionBlock *block, size_t *sz_buf,int transactions_per_block) {
  // We must subtract the size of the pointer, the static block does not have
  // the pointer
  *sz_buf = get_transaction_block_size(transactions_per_block) - sizeof(Transaction *);
...
void compute_sha256(const TransactionBlock *block, char *output,int transactions_per_block) {
